...
static int gpio_init(void) {
  ...
  sunxi_gpio_set_pull(SUNXI_GPE(1), SUNXI_GPIO_PULL_UP);
    ...
    (defined(CONFIG_MACH_SUN4I) || defined(CONFIG_MACH_SUN7I) ||               \
     defined(CONFIG_MACH_SUN8I_R40))
...
#error Unsupported console port number. Please fix pin mux settings in board.c
   ...
   * detected value.
   */
  if (IS_ENABLED(CONFIG_SUN50I_GEN_H6) || IS_ENABLED(CONFIG_SUN50I_GEN_NCAT2)) {
...
static int sunxi_get_boot_source(void) {
   ...
   */
  if (IS_ENABLED(CONFIG_MACH_SUNIV) && !IS_ENABLED(CONFIG_XPL_BUILD))
    return SUNXI_BOOTED_FROM_MMC0;
...
u32 spl_boot_device(void) { return sunxi_get_boot_device(); }
...
void reset_cpu(void) {
  ...
  static const struct sunxi_wdog *wdog = (struct sunxi_wdog *)SUNXI_R_WDOG_BASE;
#else
  static const struct sunxi_wdog *wdog =
      ((struct sunxi_timer_reg *)SUNXI_TIMER_BASE)->wdog;
#endif
