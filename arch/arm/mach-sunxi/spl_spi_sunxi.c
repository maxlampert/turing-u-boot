...
static void spi0_enable_clock(void) {
  ...
  if (IS_ENABLED(CONFIG_SUN50I_GEN_H6) || IS_ENABLED(CONFIG_SUNXI_GEN_NCAT2))
    ...
    setbits_le32(SUN6I_BUS_SOFT_RST_REG0, (1 << AHB_RESET_SPI0_SHIFT));
  ...
  if (IS_ENABLED(CONFIG_MACH_SUNIV)) {
    ...
    if (!IS_ENABLED(CONFIG_SUNXI_GEN_NCAT2)) {
      ...
      writel(SPI0_CLK_DIV_BY_4,
             base + (is_sun6i_gen_spi() ? SUN6I_SPI0_CCTL : SUN4I_SPI0_CCTL));
    }
  ...
  if (is_sun6i_gen_spi()) {
    ...
    setbits_le32(base + SUN6I_SPI0_GCR,
                 SUN6I_CTL_MASTER | SUN6I_CTL_ENABLE | SUN6I_CTL_SRST);
    /* Wait for completion */
    ...
    setbits_le32(base + SUN4I_SPI0_CTL, SUN4I_CTL_MASTER | SUN4I_CTL_ENABLE |
                                            SUN4I_CTL_TF_RST |
                                            SUN4I_CTL_RF_RST);
  }
}
...
static void spi0_disable_clock(void) {
  ...
  if (is_sun6i_gen_spi())
    clrbits_le32(base + SUN6I_SPI0_GCR, SUN6I_CTL_MASTER | SUN6I_CTL_ENABLE);
  else
    clrbits_le32(base + SUN4I_SPI0_CTL, SUN4I_CTL_MASTER | SUN4I_CTL_ENABLE);
  ...
  if (IS_ENABLED(CONFIG_SUN50I_GEN_H6) || IS_ENABLED(CONFIG_SUNXI_GEN_NCAT2))
    ...
    clrbits_le32(SUN6I_BUS_SOFT_RST_REG0, (1 << AHB_RESET_SPI0_SHIFT));
}
...
static void spi0_init(void) {
  ...
  else if (IS_ENABLED(CONFIG_MACH_SUNIV) || IS_ENABLED(CONFIG_MACH_SUN8I_R528))
...
static void sunxi_spi0_xfer(const u8 *txbuf, u32 txlen, u8 *rxbuf, u32 rxlen,
                            ulong spi_ctl_reg, ulong spi_ctl_xch_bitmask,
                            ulong spi_fifo_reg, ulong spi_tx_reg,
...
static void spi0_xfer(const u8 *txbuf, u32 txlen, u8 *rxbuf, u32 rxlen) {
  ...
  if (is_sun6i_gen_spi()) {
    sunxi_spi0_xfer(txbuf, txlen, rxbuf, rxlen, base + SUN6I_SPI0_TCR,
                    SUN6I_TCR_XCH, base + SUN6I_SPI0_FIFO_STA,
                    base + SUN6I_SPI0_TXD, base + SUN6I_SPI0_RXD,
                    base + SUN6I_SPI0_MBC, base + SUN6I_SPI0_MTC,
                    base + SUN6I_SPI0_BCC);
  } else {
    sunxi_spi0_xfer(txbuf, txlen, rxbuf, rxlen, base + SUN4I_SPI0_CTL,
                    SUN4I_CTL_XCH, base + SUN4I_SPI0_FIFO_STA,
                    base + SUN4I_SPI0_TX, base + SUN4I_SPI0_RX,
                    base + SUN4I_SPI0_BC, base + SUN4I_SPI0_TC, 0);
  }
}
