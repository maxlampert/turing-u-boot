...
int env_get_id(void) { return env_id; }
...
int env_set_force(const char *varname, const char *varvalue) {
  ...
  if (varvalue == NULL || varvalue[0] == '\0')
    return env_do_env_set(0, 2, (char *const *)argv, H_PROGRAMMATIC | H_FORCE);
  else
...
int env_get_yesno(const char *var) {
  ...
  if (s == NULL)
    return -1;
  return (*s == '1' || *s == 'y' || *s == 'Y' || *s == 't' || *s == 'T') ? 1
                                                                         : 0;
}

bool env_get_autostart(void) { return env_get_yesno("autostart") == 1; }
...
char *env_get_default(const char *name) {
  ...
  ret = env_get_default_into(name, (char *)(gd->env_buf), sizeof(gd->env_buf));
  if (ret >= 0)
    return (char *)(gd->env_buf);
...
void env_set_default(const char *s, int flags) {
  ...
  flags |= H_DEFAULT;
  if (himport_r(&env_htab, default_environment, sizeof(default_environment),
                '\0', flags, 0, 0, NULL) == 0) {
...
int env_set_default_vars(int nvars, char *const vars[], int flags) {
  ...
  flags |= H_DEFAULT;
  return himport_r(&env_htab, default_environment, sizeof(default_environment),
                   '\0', flags, 0, nvars, vars);
}
...
int env_import(const char *buf, int check, int flags) {
  ...
  if (himport_r(&env_htab, (char *)ep->data, ENV_SIZE, '\0', flags, 0, 0,
                NULL)) {
    gd->flags |= GD_FLG_ENV_READY;
...
int env_check_redund(const char *buf1, int buf1_read_fail, const char *buf2,
                     int buf2_read_fail) {
...
int env_import_redund(const char *buf1, int buf1_read_fail, const char *buf2,
                      int buf2_read_fail, int flags) {
  env_t *ep;
  int ret;
...
void env_import_fdt(void) {
  ...
  for (res = ofnode_first_property(node, &prop); !res;
